...
awk '{
    ...
    for (i = 4; i <= n-3; i++) {
	if (chr[i+3]!=chr[i]) {
		print window[i] "\t" start[i-3] "\t" end[i] 
	}else if(chr[i-3]!=chr[i]){print window[i] "\t" start[i] "\t" end[i+3]
	}else{ print window[i] "\t" start[i-3] "\t" end[i+3]
	}
...
Rscript plot_cnv_MOBIDIC.R tmp.plot_cnv.$sample.txt

#DEBUG PURPOSE exit 0
