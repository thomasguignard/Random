...
if [ ! -d $FOLDER/fast5/ ]; then
	...
	echo -e "\nguppy_basecaller --input_path $FOLDER/fast5/  --save_path $FOLDER/FASTQ/  --cpu_threads_per_caller 1 --num_callers 11 -m /opt/ont/guppy/data/template_r9.4.1_450bps_hac.jsn   -c /opt/ont/guppy/data/dna_r9.4.1_450bps_hac.cfg" >> $logfile
	guppy_basecaller --input_path $FOLDER"/fast5/"  --save_path $FOLDER"/FASTQ/"  --cpu_threads_per_caller 1 --num_callers 11 -m /opt/ont/guppy/data/template_r9.4.1_450bps_hac.jsn   -c /opt/ont/guppy/data/dna_r9.4.1_450bps_hac.cfg 2>> $logfile
	if [ $? -ne 0 ];then
...
if [ ! -e $FOLDER/FASTQ/ALL/ALL.fastq ]; then
	...
	echo "minimap2 -a -x map-ont ~/resources/$2/$2.fa $FOLDER/FASTQ/ALL/ALL.fastq -t 10 | samtools sort -@ 10 -o $FOLDER/BAM_$2/ALL_minimap2.bam" >> $logfile
	minimap2 -a -x map-ont ~/resources/$2/$2.fa $FOLDER"/FASTQ/ALL/ALL.fastq" -t 10 | samtools sort -@ 10 -o $FOLDER"/BAM_$2/ALL_minimap2.bam" 2>> $logfile
