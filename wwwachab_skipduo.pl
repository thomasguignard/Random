...
########## additionnal analysis in TRIO or affected context according to family genotype + CNV
		...
		# skip variant with case 0/0 genotype in trio or duo context
		if (defined $skipCaseWT){
			if($finalSortData[$dicoColumnNbr{"Genotype-".$case}] eq "0/0" && ( defined $trio || $dad ne "" || $mum ne "") ){
				next;
			}
		}
